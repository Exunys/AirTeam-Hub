local library = {
	Version = string.gsub(game:HttpGetAsync("https://raw.githubusercontent.com/Exunys/AirTeam-Hub/main/Resources/Extras/Version.txt"), "%s", ""),
	...
	colors = {
		main = Color3.fromHex("23A0FF"),
		background = Color3.fromHex("11182A"),
		outerBorder = Color3.fromHex("162943"),
		innerBorder = Color3.fromHex("2C4168"),
		topGradient = Color3.fromHex("1B242F"),
		bottomGradient = Color3.fromHex("202B42"),
		sectionBackground = Color3.fromHex("0E141C"),
		section = Color3.fromHex("ADC7FF"),
		otherElementText = Color3.fromHex("54637D"),
		elementText = Color3.fromHex("7692B8"),
		elementBorder = Color3.fromHex("111D26"),
		selectedOption = Color3.fromHex("558AC2"),
		unselectedOption = Color3.fromHex("4E6EA0"),
		hoveredOptionTop = Color3.fromHex("4885A0"),
		unhoveredOptionTop = Color3.fromHex("407495"),
		hoveredOptionBottom = Color3.fromHex("38667D"),
		unhoveredOptionBottom = Color3.fromHex("5471C4"),
		tabText = Color3.fromHex("C9DFF1")
	},
	gui_parent = (function()
		...
		return warn("[AirTeam]: Couldn't find a place to store the GUI!")
	end)(),
	colorpicker = false,
				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder("./AirTeam")
							if fold then
							else
								makefolder("./AirTeam")
							end
							fold = isfolder("./AirTeam/Themes")
							if fold then
							else
								makefolder("./AirTeam/Themes")
							end
							fold = isfolder("./AirTeam/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./AirTeam Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./AirTeam/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./AirTeam/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./AirTeam/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	local windowName = "AirTeam Hub"
	library.WorkspaceName = options.Name
	options.Name = windowName
	if windowName and #windowName > 0 and library.WorkspaceName == "AirTeam" then
		library.WorkspaceName = convertfilename(windowName, library.WorkspaceName)
	end
	local AirTeamLibrary = Instance_new("ScreenGui")
	library.MainScreenGui, MainScreenGui = AirTeamLibrary, AirTeamLibrary
	...
	AirTeamLibrary.Name = "     "
	AirTeamLibrary.Parent = library.gui_parent
	AirTeamLibrary.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	AirTeamLibrary.DisplayOrder = 10
	AirTeamLibrary.ResetOnSpawn = false
	main.Name = "main"
	main.Parent = AirTeamLibrary
	...
	headline.Text = (windowName and tostring(windowName)) or "N/A"
	...
	function windowFunctions:CreateTab(options, ...)
		...
		local tabName = options.Name or "N/A"
		...
		newTab.Name = removeSpaces((tabName and tostring(tabName):lower() or "N/A") .. "Tab")
		...
		if image then
			newTab.Image = image
			newTab.ImageColor3 = Color3.new(0.8, 0.8, 0.8)
			...
			newTab.Text = (tabName and tostring(tabName)) or "N/A"
		...
		newTabHolder.Name = removeSpaces((tabName and tabName:lower()) or "N/A") .. "TabHolder"
		...
		function tabFunctions:CreateSection(options, ...)
			...
			local sectionName, holderSide = options.Name or "N/A", options.Side
			...
			newSection.Name = removeSpaces((sectionName and sectionName:lower() or "N/A") .. "Section")
			...
			sectionHeadline.Text = (sectionName and sectionName or "N/A")
			...
			function sectionFunctions:AddToggle(options, ...)
				options = (options and type(options) == "string" and resolvevararg("Tab", options, ...)) or options
				local toggleName, alreadyEnabled, callback, flagName = assert(options.Name, "Unassigned \"Name\" property for toggle!"), options.Value or options.Enabled, options.Callback, options.Flag or (function()
				...
				newToggle.Name = removeSpaces((toggleName and toggleName:lower() or "N/A") .. "Toggle")
				...
				toggleHeadline.Text = toggleName or "N/A"
			...
			function sectionFunctions:AddButton(...)
				...
				for _, options in next, args do
					...
					realButton.Text = (buttonName and tostring(buttonName)) or "N/A"
					...
					newButton.Name = removeSpaces((buttonName and buttonName:lower() or "N/A") .. "Holder")
			...
			function sectionFunctions:AddTextbox(options, ...)
				...
				local textboxName, presetValue, placeholder, callback, flagName = assert(options.Name, "Unassigned \"Name\" property for textbox!"), options.Value, options.Placeholder, options.Callback, options.Flag or (function()
				...
				newTextbox.Name = removeSpaces((textboxName and textboxName:lower()) or "N/A") .. "Holder"
				...
				textboxHeadline.Text = (textboxName and tostring(textboxName)) or "N/A"
			...
			function sectionFunctions:AddKeybind(options, ...)
				...
				local keybindName, presetKeybind, callback, presscallback, flag = assert(options.Name, "Unassigned \"Name\" property for keybind!"), options.Value, options.Callback, options.Pressed, options.Flag or (function()
					library.unnamedkeybinds = 1 + (library.unnamedkeybinds or 0)
					return "Keybind" .. tostring(library.unnamedkeybinds)
				end)()
				...
				keybindHeadline.Text = (keybindName and tostring(keybindName)) or "N/A"
			...
			function sectionFunctions:AddSlider(options, ...)
				...
				local sliderName, maxValue, minValue, presetValue, callback, flagName = assert(options.Name, "Unassigned \"Name\" property for slider!"), assert(options.Max, "Missing Max for new slider."), assert(options.Min, "Missing Min for new slider."), options.Value, options.Callback, options.Flag or (function()
			...
			function sectionFunctions:AddSearchBox(options, ...)
				...
				local dropdownName, listt, val, callback, flagName = assert(options.Name, "Unassigned \"Name\" property for searchbox!"), assert(options.List, "Missing List for new searchbox."), options.Value, options.Callback, options.Flag or (function()
				...
				dropdownHeadline.Text = (dropdownName and tostring(dropdownName)) or "N/A"
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./AirTeam") then
						makefolder("./AirTeam")
					end
					local common_string = "./AirTeam/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./AirTeam/" .. tostring(custom_workspace or library.WorkspaceName)
						end
						...
						assert(isfolder(common_string), "Couldn't create folder: " .. tostring(library.WorkspaceName or "N/A"))
					...
					local function savestuff(s, get)
						if s == "..." or dropdownSelection.Text == "..." then return end
			...
			function sectionFunctions:AddColorpicker(options, ...)
				...
				local colorPickerName, presetColor, callback, flagName = assert(options.Name, "Unassigned \"Name\" property for colorpicker!"), options.Value, options.Callback, options.Flag or (function()
				...
				colorPickerHeadline.Text = colorPickerName or "N/A"
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local persistoptions = {
			...
			Flags = true,
			Suffix = "Config",
			Workspace = library.WorkspaceName,
			Desginer = true
		}

		local daaata = {{"AddPersistence", "__Designer.Persistence.WorkspaceProfile", filessection, persistoptions}, {"AddButton", "__Designer.Button.TerminateGUI", settingssection, {{
			Name = "Exit",
		...
		}}, {"AddLabel", "__Designer.Label.GameName", settingssection, {
			Name = "Game Loaded: "..library.WorkspaceName
		}}, {"AddLabel", "__Designer.Label.Version", settingssection, {
			Name = "AirTeam Hub V"..library.Version.." © 2023 - "..os.date("%Y")
		}}}
...
-- Forked from https://www.roblox.com/library/7657867786/Pepsis-UI-Library
